...
<c:if test="${empty eventId}">	
	<a href="<tags:actionURL actionId="${actionId}" startMode="${empty startMode? '':startMode}" idParam="${empty idParam? '':idParam}" parameters="${empty parameters? '':parameters}"/>" onClick="javascript:${javascript}" class="${(fn:length(buttonText) < 7) ? 'eventButtonSmall ' : (fn:length(buttonText) < 10) ? 'eventButtonMedium ' : (fn:length(buttonText) < 13) ? 'eventButtonLarge ' : 'eventButtonXLarge '} ${empty className ? 'eventButton':className}">${buttonText}</a>
</c:if>
</c:otherwise>
</c:choose>
