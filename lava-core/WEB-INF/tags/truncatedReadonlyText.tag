    ...
    <span class="${styleClass} ${alignment == 'longLeft' ? 'readonlyLongLeft' : (alignment == 'left' ? 'readonlyLeft' : '')}" >
    	...
    	<c:choose>
    		...
    		<c:when test="${not empty list[status.value]}">
    			<%--if the list item is greater than 255 characters that is fine....we really shouldn't be in this situation with this tag --%>
				${fn:replace(list[status.value],newline,"<br>")}
			</c:when>
			<c:when test="${not empty status.value && fn:length(status.value) > 255}">
	        	${fn:replace(fn:substring(status.value,0,254),newline,'<br>')}&nbsp;[...]
			</c:when>
			<c:otherwise>
