...
<%-- datetimeTextBox
     ...
     differs from dateTextBox in that is handles time in the format h:mma
--%>
...
<spring:bind path="${property}">
    ...
    <input type="text" name="${status.expression}" id="${fieldId}" value="${status.value}" autocomplete="off" class="${styleClass}" ${attributesText} 
    	...
    	onchange="javascript:validateDateTimeEntry(document.getElementById('${fieldId}'));return false;" onBlur="javascript: validateDateTimeEntry(document.getElementById('${fieldId}'));return false;">
    <A href=""  onClick="submitted=true;cal_${fieldId}.select(document.getElementById('${fieldId}'),'${fieldId}_Calendar','MM/dd/yyyy h:mma'); return false;"  NAME="${fieldId}_Calendar" ID="${fieldId}_Calendar">
