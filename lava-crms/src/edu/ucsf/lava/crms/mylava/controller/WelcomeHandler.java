...
public class WelcomeHandler extends LavaComponentHandler {


	public WelcomeHandler() {
		super();
		// must handle the refresh event even though there is nothing to refresh, to accomodate
		// project context changes which stay within the flow and do a refresh after the project
		// context has changed
		defaultEvents = new ArrayList(Arrays.asList(new String[]{"refresh"}));		
		this.setDefaultObjectName("welcome");
		Map handledObjects = new HashMap<String,Class>();
		handledObjects.put("welcome", EntityPlaceholder.class);
	...
	public Map getBackingObjects(RequestContext context, Map components) {
		...
		model.put("welcome", new EntityPlaceholder());
		return model;
	}
