...
public class CrmsAuthUserContextFilter implements Filter, ManagersAware {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if(user!=null && user.isAuthUserContextInit()){
			...
			if(userProjectAccess.size() == 1 && ! user.getAllProjectAccess() && !userProjectAccess.get(0).equals(CrmsAuthUser.PROJECT_LIST_PLACEHOLDER)){
				CrmsSessionUtils.setCurrentProject(sessionManager,httpServletRequest, userProjectAccess.get(0));
				logger.info("set CurrentProject=" + userProjectAccess.get(0));
			}
		}
