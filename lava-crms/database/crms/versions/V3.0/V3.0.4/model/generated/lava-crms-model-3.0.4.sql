...
USE `lava_crms`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `lava_crms`.`lq_view_demographics` AS select `lava_crms`.`patient`.`PIDN` AS `PIDN_demographics`,`lava_crms`.`patient`.`DOB` AS `DOB`,`lava_crms`.`patient`.`AGE` AS `AGE`,`lava_crms`.`patient`.`Gender` AS `Gender`,`lava_crms`.`patient`.`Hand` AS `Hand`,`lava_crms`.`patient`.`Deceased` AS `Deceased`,`lava_crms`.`patient`.`DOD` AS `DOD`,`lava_crms`.`patient`.`PrimaryLanguage` AS `PrimaryLanguage`,`lava_crms`.`patient`.`TestingLanguage` AS `TestingLanguage`,`lava_crms`.`patient`.`TransNeeded` AS `TransNeeded`,`lava_crms`.`patient`.`TransLanguage` AS `TransLanguage` from `lava_crms`.`patient` where (`lava_crms`.`patient`.`PIDN` > 0);
