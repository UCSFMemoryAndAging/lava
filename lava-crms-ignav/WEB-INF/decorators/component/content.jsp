...
<%-- SWITCHABLE: we don't want instrument CRUD to be modal, but other modules are still desired to be modal --%>
<meta name="isInstrument" content="${isInstrument}">
<meta name="componentView" content="${componentView}"/> <%-- now used in both main and modal --%>
<%--<c:if test="${isInstrument || (componentView == 'edit' && component != 'reportSetup') || componentView == 'add' || componentView == 'delete'}">--%>
<c:if test="${not isInstrument && ((componentView == 'edit' && component != 'reportSetup') || componentView == 'add' || componentView == 'delete')}">
	...
	<%-- now used in both main and modal --%>
...
<%-- EMORY change: display info header only once when multiple info messages exist --%>
<c:set var="headerAlreadyDisplayed" value="false"/>
<c:forEach items="${status.errors.globalErrors}" var="error"> 
	<c:if test="${fn:startsWith(error.code, 'info.') && not headerAlreadyDisplayed}">
